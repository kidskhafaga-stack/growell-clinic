...
| `IMT_record_matrix.md` | **مراجعة الملف الطبي** — IMT.04–09 ومعاهم ACT.04/ACT.15 وICD.06/07/15 |
...
للموافقة، و`IMT` (في `IMT_record_matrix.md`) ومعاه `ACT.15` و`ICD.06`/`07`/`15`
من ناحية السجل. **فاضل:** `CSS`، وباقي `ACT`، و`MMS`.
