...
### ~~وتقرير الحالة برضه~~ — **اتقفلت**

`patients/report` — «التقرير الطبي الشامل» — كان بيجمّع: بيانات · قايمة
المشاكل · النمو · التطعيمات · آخر ١٠ زيارات · آخر روشتة.

**ومكانش فيه إقامات ولا عمليات**، فالورقة اللي المفروض تلخّص الملف كانت
بتلخّص نُصّه — وده الشكل اللي السجل بيخرج بيه من المبنى أكتر من أي شكل تاني.

بقى فيه قسمين كمان: **الإقامات** و**العمليات**.

---

## ٢) ACT.15 — ملخّص الخروج — **اتقفلت**
...
## ٣) IMT.09 — مراجعة اكتمال الملف — **اتقفلت**
...
> ✅ **اتعملت.** `/reports/record-review` — جولة بتسحب عيّنة وتقراها وتكتب
> اللي لقته.
>
> **والقاعدة: البرنامج ما بيخترعش تعريف للملف الكامل.** ست فحوصات، كل واحد
> حقيقة البرنامج بيستنتجها أصلاً لشاشة من شاشاته مقابل بند متطبّق فيه —
> `ACT.15` · `GSR.16` · `SAS.06` · `PCC.08` · `ICD.06` — والبند مكتوب جنب كل
> سطر. **ملاحظات، مش أحكام**: الحكم شغل اللجنة، وده بالظبط اللي (د) حاطط ناس
> في الأوضة عشانه.
>
> **مخزّن مش مشتق** — الاستثناء الوحيد في البرنامج كله، وبيستاهله: مراجعة
> مارس لازم تفضل تقول اللي لقته في مارس، وإلا عيادة صلّحت ١٢ ملف هتلاقي
> نفسها بدليل إنها ما لقتش حاجة.
>
> **والعيّنة مقسّمة** عشان (ب) و(ج): واحد لكل نوع إدخال وواحد لكل شخص
> بيكتب، وبعدين عشوائي للـ٥٪ — **وبتزيد عن الـ٥٪ بدل ما تشيل حد**.
>
> و(هـ) الوضوح: **البرنامج ما بيحسبش فيه حاجة وبيقول كده** — خانة بيملاها
> حد. و(و): «at least quarterly» زي ما البند قال، **وعيادة عمرها ما عملت
> مراجعة مش «متأخرة»**، دي عيادة ما بدأتش.
>
> ودليل ٣ (الإدارة اتبلّغت) ختم، ودليل ٤ (الإجراء التصحيحي) نص بيترفض
> فاضي.
...
### ~~١ — الملف ما بيشوفش الإقامات (IMT.08 دليل ٤ و٥)~~ — **اتقفلت**
...
> ✅ **اتعملت.** تبويب «الإقامات» في الملف (`beds.stays_for` + لوحة في
> `profile.html`): كل إقامة — المفتوحة والمقفولة — بتاريخها وسببها والطبيب
> المسؤول، **والأسرّة بالترتيب اللي حصل**، والمآل وملاحظة الخروج، والعمليات
> اللي اتعملت جواها، وعدد ملاحظات المرور. وكل سطر بيوصّل شاشة الإقامة
> نفسها، اللي بترسم إقامة مقفولة read-only من قبل كده.
>
> **والتقرير الطبي كمان.** `patients/report` بقى فيه قسمين: الإقامات
> (بطرفين التاريخ، والسبب، والأقسام اللي عدّى عليها كل واحد مرّة، والمآل)،
> والعمليات — **من غير قص**: عشر زيارات «أخيرة» عيّنة مكتوب عليها كده، لكن
> عشرة من أربعتاشر عملية تاريخ جراحي بيبان كامل وهو مش كامل.

### ~~٢ — مفيش ملخّص خروج (ACT.15)~~ — **اتقفلت**
...
> ✅ **اتعملت.** `DischargeSummary` + `app/utils/discharge_summary.py`:
> **تلاتة من التسعة بيتقروا من السجل** (السبب · الفحوصات في نافذة الإقامة ·
> العمليات · الأدوية قبل وأثناء · اسم اللي خرّجه)، و**ستة بيتكتبوا**
> (التشخيص النهائي · اللي اتلاقى · حالة الطفل · الأكل · الدوا · المتابعة).
> والحالة مشتقّة: `none` · `short` · `complete` — و`short` مسمّاة كده عشان
> ما تتقراش خلصانة.
>
> **صف مش أعمدة**، فالغياب هو الاستنتاج. **ووقت الكتابة ختم لوحده** عن وقت
> الخروج، والتأخير **بيتقاس ما بيتحكمش عليه**. **وتسليم النسخة للأهل ختم
> تالت** (دليل ٤). وشاشة طباعة بتقرا المشتق **لحظة الطبع**.
>
> **ومفيش حاجة بترفض خروج** — الفجوة بتتعرض على شاشة الإقامة وعلى ملف
> الطفل، وبتفضل تتعرض.

### ~~٣ — مفيش مراجعة اكتمال الملف (IMT.09)~~ — **اتقفلت**

آخر واحد **مش لأنه أقل أهمية، لكن لأن التنتين اللي فوقه شرطه**: مراجعة
اكتمال على ملف نُصّه مش ظاهر هتقيس النُص. **والتنتين اتقفلوا دلوقتي**، فده
بقى البند الجاي.

وده كان اللي البرنامج يقدر يحمله **كامل** — العيّنة والنسبة والربع سنوي
والاكتمال كلهم حساب — واتعمل.

**وباقي من البند حاجتين مش شغل برنامج:** السياسة المعتمدة (دليل ١) ووعي
الموظفين بيها (دليل ٢). دول ورق المستشفى، والبرنامج ما بيدّعيش إنه بيغطّيهم.

---

## اللي فاضل من فصل IMT كله

| البند | الحالة |
|---|---|
| `IMT.04` · GSR.29 — الاختصارات | ❌ مفيش قايمة معتمدة ولا ممنوعة |
| `IMT.05` دليل ٥ — إقرار سرّية في ملف كل موظف | ❌ |
| `IMT.07` — مدة حفظ السجل وإجراءات الإتلاف | ❌ (اللي موجود مدة حفظ **النُسخ الاحتياطية**، حاجة تانية) |
...
**بس «مغطّى» كانت كبيرة أوي على اللي كان متحقَّق ساعتها.** النص وصل، والمراجعة
دي كانت الإجابة الحقيقية: `IMT.08` نُصّه، و`IMT.04` و`IMT.07` و`IMT.09` مش
موجودين.

**واتغيّر من ساعتها:** `IMT.08` اتقفل (الملف بقى يشوف الإقامات، والتقرير كمان)،
و`ACT.15` اتعمل، و`IMT.09` اتعمل. **واللي لسه:** `IMT.04` الاختصارات ·
`IMT.05` دليل ٥ إقرار السرّية · `IMT.07` مدة حفظ السجل وإتلافه.
