...
def test_a_banded_brand_says_how_many_bands(catalogue):
    ...
    # Prevenar 13 carries its own leaflet bands now — the catch-up its label
    # states in numbers. It used to carry only the pneumococcal ceiling, which
    # was a vaccine-wide row, and that row has since moved to the guidelines
    # that state it rather than travelling with every clinic.
    #
    # Asserted as "there are some", not as a count: the count is exactly what
    # the docstring above says a test must not pin.
    bands = rows["Prevenar 13"]["R1_age_bands"]
    assert bands and bands != "0", \
        f"Prevenar 13 lost its leaflet's own catch-up: {bands!r}"
