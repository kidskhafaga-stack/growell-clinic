...
class NursingAssessment(db.Model):
    ...
    admission_id = db.Column(db.Integer, db.ForeignKey("admissions.id"),
                             nullable=False, index=True)
    #: **والافتراض «إعادة» مش «أول».**
    #:
    #: ده عكس اللي كتبته أول مرة، وكنس الطفرات هو اللي وقفني عليه:
    #: الإنذار هنا هو «مفيش تقييم أول». فصف اتكتب من غير نوع لو بقى
    #: «أول» بيسكّت الإنذار ده على إقامة محدّش قيّمها — والافتراض
    #: الآمن دايماً هو اللي **بيفضل بيسأل**، مش اللي بيقفل السؤال.
    #:
    #: و`record()` بتبعت النوع صراحةً في كل الحالات اللي من الشاشة،
    #: فالافتراض ده مالوش أثر غير على صف جاي من استيراد أو تعديل مباشر
    #: — وهو بالظبط الصف اللي مش عايزينه يسكّت حاجة.
    kind = db.Column(db.String(14), nullable=False, default=REASSESSMENT,
                     index=True)
